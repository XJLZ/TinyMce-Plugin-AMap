tinymce.PluginManager.add('gdmap', function(editor, url) {
	var pluginName='插入高德地图';
	var baseURL=tinymce.baseURL;
	var iframe1 = baseURL+'/plugins/gdmap/map.html';
	var gdmap_width = function (editor) {
		return editor.getParam('gdmap_width', 560);
    };
    var gdmap_height = function (editor) {
		return editor.getParam('gdmap_height', 362);
    };
	window.tinymceLng='';
	window.tinymceLat='';
	var openDialog = function() {
		return editor.windowManager.openUrl({
			...
			onAction: function (api, details) {
				switch (details.name) {
					case 'save':
						html='<iframe src="'+baseURL+'/plugins/gdmap/gd.html?center='+tinymceLng+'%2C'+tinymceLat+'&zoom=17&width='+(gdmap_width(editor)-2)+'&height='+(gdmap_height(editor)-2)+'" frameborder="0" style="width:'+gdmap_width(editor)+'px;height:'+gdmap_height(editor)+'px;">';
	...
	editor.ui.registry.addMenuItem('gdmap', {
		text: pluginName,
		onAction: function() {
			openDialog();
		}
	...
	return {
		getMetadata: function() {
			return  {
				name: pluginName,
				url: "https://lbs.amap.com/api/javascript-api/guide/abc/prepare",
			};
		}
	};
});
